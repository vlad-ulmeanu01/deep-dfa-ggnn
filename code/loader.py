...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, dset_type: str):
        ...
        with open(f"{utils.TEST_FOLDER}/train_test_split_{utils.LOADER_NUM_SAMPLES}_samples.json") as fin:
        ...
        self.df_nodes = pd.read_csv(f"{utils.TEST_FOLDER}/nodes_{utils.LOADER_NUM_SAMPLES}_samples.csv")
        self.df_nodes = self.df_nodes[self.df_nodes["graph_id"].isin(ht_tt_split_graph_ids[dset_type])]

        # {innode}, {outnode}, [id_x], [id_y], graph_id
        self.df_edges = pd.read_csv(f"{utils.TEST_FOLDER}/edges_{utils.LOADER_NUM_SAMPLES}_samples.csv")
        ...
        # tine top K prop names pentru "api", "datatype", "literal", "operator" (aici sunt mult mai putin de {utils.LOADER_NUM_SAMPLES}).
        ...
        self.graphs_parent_list = {
            graph_id: [[nod] for nod in range(self.cnt_nodes_per_graph[graph_id])] # il includem si pe nod in lista.
            for graph_id in self.graph_ids
        }
    ...
    def __getitem__(self, ind: int):
        ...
        rep = torch.zeros(self.cnt_nodes_per_graph[graph_id], utils.TOP_K_PROP_NAMES * len(self.ht_used_property_names), device = utils.DEVICE)
